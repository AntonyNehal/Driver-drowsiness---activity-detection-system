...
# EAR & MAR thresholds (adjusted for more accuracy)
EYE_THRESH = 0.25
MAR_THRESH = 0.75  # Increased to reduce false yawning alerts
YAWN_LIMIT = 7     # Increased yawning count to trigger alarm
FRAME_CHECK = 15   # Increased frame check for drowsiness
...
# Functions for EAR & MAR calculations
...
# Function to play alert sound in background thread
...
# Function to fetch and speak nearby hotels
...
while cap.isOpened():
    ...
    # Distraction detection (reduce false alerts using confidence threshold)
    for result in results:
        for box in result.boxes:
            label = result.names[int(box.cls[0])]
            confidence = float(box.conf[0])

            if label == "cell phone" and confidence > 0.6:  # High confidence for phone detection
