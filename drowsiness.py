...
#         data = response.json()
#         hotels = [hotel["name"] for hotel in data.get("results", [])]
#         return [hotels[0]] if hotels else ["No hotel found nearby"]
#     except:
#         return ["Error fetching hotel"]
...
#             elif label in ["sandwich", "hot dog", "pizza"] and confidence > 0.5:  # "sandwich" might detect burgers
#                 handle_eating_alert()
...
# Google Places API Key
API_KEY = "AIzaSyAyl1IIUMNQgdlY-Ni3lxUUwFOjMQmI9sE"
...
# Get a single nearby hotel
def get_nearby_hotel(lat, lng):
    url = (f"https://maps.googleapis.com/maps/api/place/nearbysearch/json"
           f"?location={lat},{lng}&radius=5000&type=lodging&key={API_KEY}")
    try:
        response = requests.get(url)
        data = response.json()
        return data.get("results", [{}])[0].get("name", "No hotels found")
    except:
        return "Error fetching hotels"


# Global variable to store the hotel name
nearby_hotel = "Searching for hotels..."

# Speak and store a nearby hotel name
def speak_nearby_hotel():
    def fetch_and_speak():
        global nearby_hotel
        lat, lng = get_current_location()
        if lat and lng:
            nearby_hotel = get_nearby_hotel(lat, lng)
        else:
            nearby_hotel = "Location fetch failed"
        engine.say("Nearest hotel is " + nearby_hotel)
        engine.runAndWait()
    threading.Thread(target=fetch_and_speak, daemon=True).start()


# # Speak a nearby hotel
# def speak_nearby_hotel():
#     def fetch_and_speak():
#         global nearby_hotel
#         lat, lng = get_current_location()
...
while cap.isOpened():
    ...
    if show_alert:
        ...
        # Display the nearest hotel name
        cv2.putText(frame, f"Nearest Hotel: {nearby_hotel}", (50, 100),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
